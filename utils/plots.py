...
def plot_alignment(input_img, preds):
    if preds is not None:
        # Extract 2D points from the landmarks
        ...
        for pred in preds:
            ...
            cv2.circle(input_img, (x, y), 2, (0, 255, 0), -1)  # Draw green circles for landmarks
            # Find the convex hull of the points (outer boundary)
            
            hull = cv2.convexHull(points)

            # Draw the convex hull as a polyline (closed curve)
            cv2.polylines(input_img, [hull], isClosed=True, color=(255, 0, 0), thickness=2)  # Blue curve for outer boundary
